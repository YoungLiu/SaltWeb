...
def genSession(SID, Username, ShowName, LastIP, LastLocation, LastDate, Token, Lstat, kpl):
    ...
    if not Token:
        ...
        db.query(
            '''insert into login_logs (uid,ip,location,agent,token,expiry) values ("%s","%s","%s","%s","%s",NOW() + INTERVAL %d SECOND)''' % (
            SID, LoginHost, Location, Agent, Token, Expiry))
        db.query(
            '''update users set loginfo=(select id from login_logs where uid="%s" and ip="%s" and token="%s" and  status="yes" order by id desc limit 1) where id="%s"''' % (
            SID, LoginHost, Token, SID))
...
class Login:
        ...
        if uid and token:
            ...
            try:
                g = db.query('''
                    ...
                    where U.id="%s" and L.token="%s" and L.status="yes" and L.expiry>now() and U.status="yes"''' % (
                uid, token))
            except Exception, e:
            ...
            if g:
                ...
                if d.loginfo != d.LID:
                    g2 = db.query(
                        '''select L.ip,L.date from users as U left join login_logs as L on U.loginfo=L.id where U.id="%s"''' % uid)
                    d = g2[0]
                    # 这里还可以返回一个异地浏览器登录的提示
    ...
    def POST(self, *args):
        ...
        try:
            getSQL = db.query(
                '''select u.id,u.username,u.password,u.nickname,u.status,L.ip,L.location,L.date from users as u left join login_logs as L on u.loginfo=L.id where username="%s" and password=md5("%s")''' % (
                getPost.username, getPost.password))
        except:
            # 服务器(数据库)错误
            return "false"
        if getSQL:
